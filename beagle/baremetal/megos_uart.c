...
int Register_Read(unsigned int, unsigned int);
...
static void Busy_Wait(unsigned long wait)
{
...
serial_recv(void)
{
	...
	while( 1 )
	{
		if( Register_Read(UART0_BANK, UART_LSR) & 0x01 )
			break;
	}
...
static void
serial_send(unsigned int c)
{
	// 0x20 indicates that the transmit hold register is empty.
	while( 1 )
	{
		if( Register_Read(UART0_BANK, UART_LSR) & 0x20 )
			break;
	}
...
serial_flush(void)
{
	...
	while( 1 )
	{
		if( Register_Read(UART0_BANK, UART_LSR) & 0x40 )
			break;
	}
}

void
serial_hex_to_ascii_hex(unsigned int aiHex)
{
...
serial_set_pin_mux_mode(void)
{
	...
	Register_Write(MODULE_CONTROL_BANK, CONF_UART0_RXD, 0x30);
...
void
megos_UART0_send_string(char* msg)
{
